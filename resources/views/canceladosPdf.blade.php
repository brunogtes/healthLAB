...
<head>
    ...
    <title>Relatório - Lista de Exames - Cancelados</title>
    <style>
        body {
...
<body>
    ...
    <h1 style="text-align: center;">LISTA DE EXAMES - CANCELADOS</h1>
    ...
    <table>
        ...
        <tbody>
            ...
            @foreach($cancelado as $cancelados)
            <tr>

                @foreach($listaUsuarios as $usuarios)
                @if($cancelados->paciente_id == $usuarios->id)
                <th>{{$usuarios->nome}} {{$usuarios->sobrenome}}</th>
                @endif
                @endforeach

                @foreach($listaExames as $exames)
                @if($cancelados->exame_id == $exames->id)
                <th>{{$exames->descricao}}</th>
                @endif
                @endforeach


                <th>{{$cancelados->etapa}}</th>
