...
<head>
    ...
    <title>Relatório - Lista de Exames - Aguardando Resultado</title>
    <style>
        body {
...
<body>
    ...
    <h1 style="text-align: center;">LISTA DE EXAMES - AGUARDANDO RESULTADO</h1>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                @if($resultados->paciente_id == $usuarios->id)
                <th>{{$usuarios->nome}} {{$usuarios->sobrenome}}</th>
                @endif
                @endforeach

                @foreach($listaExames as $exames)
                @if($resultados->exame_id == $exames->id)
                <th>{{$exames->descricao}}</th>
                @endif
                @endforeach


                <th>{{$resultados->etapa}}</th>
