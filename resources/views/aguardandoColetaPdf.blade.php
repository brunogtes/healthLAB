...
<head>
    ...
    <title>Relatório - Lista de Exames - Aguardando Coleta</title>
    <style>
        body {
...
<body>
    ...
    <h1 style="text-align: center;">LISTA DE EXAMES - AGUARDANDO COLETA</h1>
    ...
    <table>
        ...
        <tbody>
            ...
            @foreach($coleta as $coletas)
            <tr>

                @foreach($listaUsuarios as $usuarios)
                @if($coletas->paciente_id == $usuarios->id)
                <th>{{$usuarios->nome}} {{$usuarios->sobrenome}}</th>
                @endif
                @endforeach

                @foreach($listaExames as $exames)
                @if($coletas->exame_id == $exames->id)
