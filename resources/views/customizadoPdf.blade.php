...
<head>
    ...
    <title>Relatório - Lista de Exames - Customizado</title>
    <style>
        body {
...
<body>
    ...
    <table>
        ...
        <tbody>
            ...
            @foreach($exameCustomizado as $customizados)
            <tr>

                @foreach($listaUsuarios as $usuarios)
                @if($customizados->paciente_id == $usuarios->id)
                <th>{{$usuarios->nome}} {{$usuarios->sobrenome}}</th>
                @endif
                @endforeach

                @foreach($listaExames as $exames)
                @if($customizados->exame_id == $exames->id)
                <th>{{$exames->descricao}}</th>
                @endif
                @endforeach


                <th>{{$customizados->etapa}}</th>
